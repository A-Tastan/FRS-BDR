% This function performs FRS-BDR algorithm for known number of clusters.
%
% For details, see:
%
...
%       K               :(numeric) specified number of blocks
...
%                         K x 1
%       error           :(numeric) estimation error associated to K
%
% version     : 29/11/2023
% authors     : Aylin Tastan
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [W_block_hat,v_dddot_hat,W_sim_hat,bsize_vec_hat,error] = perform_FRS_BDR_algorithm_knownK(X,K,K_max,N_cmax,min_num_samples,order_opt,sim_func_opt,fit_opt,eig_func_opt,removal_opt,TOL)
...
cand_block_size_mat = compute_candidate_block_sizes(tau,N_of,K,min_num_samples);

%% Step 2.2 : Estimating W_sim
if(~isempty(cand_block_size_mat))

    [v_dddot_hat,W_sim_hat,bsize_vec_hat,error] = estimate_vector_v_and_sim_coefficients_matrix(L_dddot,v_dddot,cand_block_size_mat,K,fit_opt);
...
if (~isempty(W_sim_hat))
    ...
    W_block_hat = compute_block_diagonal_affinity_matrix(W_dddot,W_sim_hat,bsize_vec_hat,K);
else
